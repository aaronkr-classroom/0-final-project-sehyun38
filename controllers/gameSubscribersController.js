 ...
 */
// 구독자 모델 요청
const Subscriber = require("../models/GameSubscriber");
...
module.exports = {
  ...
  indexView: (req, res) => {
    res.render("gameSubscribers/index", {
      page: "subscribers",
  ...
  new: (req, res) => {
    res.render("gameSubscribers/new", {
      page: "new-subscriber",
  ...
  create: (req, res, next) => {
    ...
    Subscriber.create(subscriberParams)
      .then((subscriber) => {
        res.locals.redirect = "/gameSubscribers";
  ...
  // show 뷰의 렌더링
  showView: (req, res) => {
    res.render("gameSubscribers/show", {
  ...
  edit: (req, res, next) => {
    ...
    Subscriber.findById(subscriberId) // ID로 데이터베이스에서 사용자를 찾기 위한 findById 사용
      .then((subscriber) => {
        res.render("gameSubscribers/edit", {
  ...
  update: (req, res, next) => {
    ...
    User.findByIdAndUpdate(subscriberId, {
      ...
      .then((user) => {
        res.locals.redirect = `/gameSubscribers/${subscriberId}`;
        res.locals.subscriber = subscriber;
